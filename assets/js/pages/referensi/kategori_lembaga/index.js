	...
	datatable = $('#table-data').DataTable({
		...
		initComplete: async function () {
			...
			$(this).on('click', '.btn-update', function (event) {
				...
				$('#ubah-id').val(data.id_kategori)
				$('#ubah-nama').val(data.nama_kategori)
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						fetch(base_url('delete'), {
							method: 'POST',
							body: { id_kategori, [CSRF.token_name]: CSRF.hash }
						}).then(res => {	// Success
							let { message, code, csrf } = res
...
const validator = {
	tambah: $('#form-tambah').validate({
		...
		messages: {
			nama: "Format Diskusi menu tidak boleh kosong",
		}
	}),
