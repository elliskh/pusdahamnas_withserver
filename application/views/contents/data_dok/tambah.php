				...
				<div class="display_form_registrasi">
					<form class="form form-horizontal" action="<?= base_url('data_dok/simpan') ?>" enctype='multipart/form-data' method="POST">
						...
						<div class="form-body">
							...
							<div class="row mb-3">
								...
								<div class="col-md-10">
									<select name="id_jenis_dokumen"
										id="jenis_dokumen" class="form-control select2">
										<option value="" disabled selected>Pilih</option>
							...
							<div class="row mb-3" style="display:none;" id="penulis">
								<div class="col-md-2">
									<label>Penulis</label>
								</div>
								<div class="col-md-10">
									<input type="text"
										id="input_penulis" class="form-control" name="penulis" placeholder="Penulis" value="<?= @$data->penulis ?>">
								</div>
							</div>
							<div class="row mb-3">
								<div class="col-md-2">
								...
								<div class="col-md-10">
									<select name="id_topik_hak[]" class="form-control select2" multiple="multiple">
										<!-- <option value="" disabled selected>Pilih</option> -->
										<?php foreach ($hak as $key => $value) { ?>
											<option value="<?= encode_id($value->id_hak) ?>" <?= in_array($value->id_hak, (array)@$data->id_topik_hak) ? 'selected' : '' ?>><?= $value->nama_hak ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
							<div class="row mb-3">
								...
								<div class="col-md-10">
									<select name="id_topik_subyek[]" class="form-control select2" multiple="multiple">
										<!-- <option value="" disabled selected>Pilih</option> -->
										<?php foreach ($subyek as $key => $value) { ?>
											<option value="<?= encode_id($value->id_subyek) ?>" <?= in_array($value->id_subyek, (array)@$data->id_topik_subyek) ? 'selected' : '' ?>><?= $value->nama_subyek ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
							...
							<div class="row mb-3">
								...
								<div class="col-md-10">
									<select name="id_lembaga[]" class="form-control select2" multiple="multiple">
										<!-- <option value="" disabled selected>Pilih</option> -->
										<?php foreach ($lembaga as $key => $value) { ?>
											<option value="<?= encode_id($value->id) ?>" <?= in_array($value->id, (array)@$data->id_lembaga) ? 'selected' : '' ?>><?= $value->nama_lembaga ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
							...
							<div class="row mb-3">
								...
								<div class="col-md-10">
									<select name="id_sumber[]" class="form-control select2" multiple="multiple">
										<!-- <option value="" disabled selected>Pilih</option> -->
										<?php foreach ($sumber as $key => $value) { ?>
											<option value="<?= encode_id($value->id) ?>" <?= in_array($value->id, (array)@$data->id_sumber) ? 'selected' : '' ?>><?= $value->nama_sumber ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
							...
							<div class="row mb-3">
								...
								<div class="col-md-10">
									<select name="id_bahasa" class="form-control select2">
										<option value="" disabled selected>Pilih</option>
										<?php foreach ($bahasa as $key => $value) { ?>
											<option value="<?= encode_id($value->id) ?>" <?= @$data->id_bahasa == $value->id ? 'selected' : '' ?>><?= $value->nama_bahasa ?></option>
										<?php } ?>
									</select>
								</div>
							</div>

							<div class="row mb-3" id="dokumen_link">
								<div class="col-md-2">
							...
							<div class="row mb-3" id="dokumen_upload">
								<div class="col-md-2">
...
<script>
	...
	$(document).ready(function () {
		if ($('#jenis_dokumen').val() == '<?php echo encode_id(2); ?>'){
			$('#penulis').show();
		}
		$('#jenis_dokumen').change(function() {
            console.log($(this).val())
            if ($(this).val() == '<?php echo encode_id(2); ?>') {
                $('#penulis').show();
            } else {
                $('#penulis').hide();
                $('#input_penulis').val(null);
            }
        });
	})
