...
$(() => {
	...
	$('#table-data').on('click', '.btn-remove', function () {
		...
		let { id, nama_audit } = data;

		Swal.fire({
			title: 'Anda yakin?',
			html: `Anda akan menghapus user "<b>${nama_audit}</b>"!`,
			...
			if (result.isConfirmed) {
				$.post(BASE_URL + 'audit_ham/deleteAuditHam/' + MENU_ID, { id, [CSRF.token_name]: CSRF.hash }).done((res) => {
					if (res.status) {
	...
	$('#table-data').on('change', '.change-active', function () {
		...
		$.post(BASE_URL + 'audit_ham/changeAuditHamActive/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
	...
	table = $('#table-data').DataTable({	    
		...
		ajax: {
			url: BASE_URL + 'audit_ham/data/' + MENU_ID,
			type: 'get',
			dataType: 'json'
		}, 
		order: [[5, 'desc']],
		...
		columns: [{
           ...
           //decode_id 
              if (type === 'display'){
                return '<a href="'+ BASE_URL + 'audit_ham/detail_auditham/' +  row.id + '" title="Detail Data Audit">'+ data + '</a>';
              }
              return data;
            }
		}, {
