...
$(() => {
	...
	$('#table-data').on('click', '.btn-remove', function () {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.post(BASE_URL + 'audit_ham/deleteDetailAudit/' + MENU_ID, { id, [CSRF.token_name]: CSRF.hash }).done((res) => {
					if (res.status) {
	...
	$('#table-data').on('change', '.change-ya-tidak', function () {
		...
		$.post(BASE_URL + 'audit_ham/changeDetailYaTidak/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
	...
	$('#table-data').on('change', '.change-active-public', function () {
		...
		$.post(BASE_URL + 'audit_ham/changeDetailActivePublic/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
    		if(res.status=='Sukses'){
    				toastrSuccess('Sukses', res.message);
    				table.ajax.reload();
    				CSRF.token_name = res.csrf.token_name;
	...
	$('#table-data').on('click', '.btn-update', function () {
		let tr = $(this).closest('tr');
		let data = table.row(tr).data();
        let url_edit  = '<?= '' . $link . '/' . $menu_id . '' ?>'.replace('tambah','edit_auditham');
	...
	table = $('#table-data').DataTable({	    
		...
		ajax: {
			url: BASE_URL + 'audit_ham/detail_data/' + MENU_ID,
			type: 'get',
			dataType: 'json'                        
		}, 
		order: [[5, 'desc']],
