...
$(() => {
	...
	$('#table-data').on('click', '.btn-remove', function () {
		...
		let { id, nama_anggaran } = data;

		Swal.fire({
			title: 'Anda yakin?',
			html: `Anda akan menghapus user "<b>${nama_anggaran}</b>"!`,
			...
			if (result.isConfirmed) {
				$.post(BASE_URL + 'anggaran_ham/deleteAnggaranHam/' + MENU_ID, { id, [CSRF.token_name]: CSRF.hash }).done((res) => {
					if (res.status) {
	...
	$('#table-data').on('change', '.change-active', function () {
		...
		$.post(BASE_URL + 'anggaran_ham/changeAnggaranHamActive/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
	...
	table = $('#table-data').DataTable({	    
		...
		ajax: {
			url: BASE_URL + 'anggaran_ham/data/' + MENU_ID,
			type: 'get',
			dataType: 'json'
		}, 
		order: [[5, 'desc']],
		...
		columns: [{
            ...
            render: function ( data, type, row ) {
              ...
              if (type === 'display'){
                return '<a href="'+ BASE_URL + 'anggaran_ham/detail_anggaranham/' +  row.id + '" title="Detail Data Anggaran">'+ data + '</a>';
              }
              return data;
            }
		}, {
