...
$(() => {
	...
	$('#table-data').on('click', '.btn-remove', function () {
		...
		let { id, deskripsi } = data;

		Swal.fire({
			title: 'Anda yakin?',
			html: `Anda akan menghapus user "<b>${deskripsi}</b>"!`,
			...
			if (result.isConfirmed) {
				$.post(BASE_URL + 'anggaran_ham/deleteDetailAnggaran/' + MENU_ID, { id, [CSRF.token_name]: CSRF.hash }).done((res) => {
					if (res.status) {
	...
	$('#table-data').on('change', '.change-ya-tidak', function () {
		let id = $(this).data('user_id')
		let checked = $(this).prop('checked');

		$.post(BASE_URL + 'anggaran_ham/changeDetailYaTidak/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
	...
	$('#table-data').on('change', '.change-active-public', function () {
		let id = $(this).data('user_id')
		let checked = $(this).prop('checked');

		$.post(BASE_URL + 'anggaran_ham/changeDetailActivePublic/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
    		if(res.status=='Sukses'){
    				toastrSuccess('Sukses', res.message);
    				table.ajax.reload();
    				CSRF.token_name = res.csrf.token_name;
	...
	$('#table-data').on('click', '.btn-update', function () {
		let tr = $(this).closest('tr');
		let data = table.row(tr).data();
        let url_edit  = '<?= '' . $link . '/' . $menu_id . '' ?>'.replace('tambah','edit_anggaranham');
	...
	table = $('#table-data').DataTable({	    
		...
		ajax: {
			url: BASE_URL + 'anggaran_ham/detail_data/' + MENU_ID,
			type: 'get',
			dataType: 'json'                        
		}, 
		order: [[5, 'desc']],
		...
		columns: [{
            ...
            if(row.tipe_daftar==2 && <?php echo $check_ss;?> > 30){
				return `                
                    <label class="custom-control-label-" for="active-${id}">${data == '1' ? 'YA' : 'TIDAK'}</label>                
                `;
            }else{
				return `
                <div class="custom-control custom-switch mb-3" dir="ltr">
                    <input type="checkbox" class="custom-control-input change-ya-tidak" name="active-${id}" id="active-${id}" data-user_id="${id}" ${data == '1' ? 'checked' : ''}>
                    <label class="custom-control-label" for="active-${id}">${data == '1' ? 'YA' : 'TIDAK'}</label>
                </div>
                `;                
            }   
            ...
            if(row.tipe_daftar==2 && <?php echo $check_ss;?> > 30){
				return `                
                    <label class="custom-control-label-" for="active-public-${id}">${data == '1' ? 'Aktif' : 'Nonaktif'}</label>                
                `;
            }else{
				return `
                <div class="custom-control custom-switch mb-3" dir="ltr">
                    <input type="checkbox" class="custom-control-input change-active-public" name="active-public-${id}" id="active-public-${id}" data-user_id="${id}" ${data == '1' ? 'checked' : ''}>
                    <label class="custom-control-label" for="active-public-${id}">${data == '1' ? 'Aktif' : 'Nonaktif'}</label>
                </div>
                `;                
            }   
