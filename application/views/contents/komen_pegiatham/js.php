...
$(() => {
	...
	$('#table-data').on('click', '.btn-remove', function () {
		...
		let { id_msg, pesan } = data;

		Swal.fire({
			title: 'Anda yakin?',
			html: `Anda akan menghapus data "<b>${pesan}</b>"!`,
			...
			if (result.isConfirmed) {
				$.post(BASE_URL + 'komen_pegiatham/deleteMsg/' + MENU_ID, { id_msg, [CSRF.token_name]: CSRF.hash }).done((res) => {
					if (res.status) {
	...
	$('#table-data').on('change', '.change-active', function () {
		...
		$.post(BASE_URL + 'komen_pegiatham/changeMsgActive/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
    		if(res.status==true){
    				toastrSuccess('Sukses', res.message);
    				table.ajax.reload();
    				CSRF.token_name = res.csrf.token_name;
	...
	table = $('#table-data').DataTable({	    
		...
		ajax: {
			url: BASE_URL + 'komen_pegiatham/data/' + MENU_ID,
			type: 'get',
			dataType: 'json'
		}, 
		order: [[7, 'desc']],
		...
		columns: [{
            ...
            render: function ( data, type, row ) {
              ...
              if (type === 'display'){
                return '<a href="'+ BASE_URL + 'home/detail_back_komham/' +  row.id + '" target="_blank">'+ data + '</a>';
              }
              return data;
            }
		}, {
				...
				let { id } = row;
                 ...
                 if(row.id == 30){
                    ...
                    <div class="custom-control custom-switch mb-3" dir="ltr">
                        <input type="checkbox" class="custom-control-input change-active" name="active-${id}" id="active-${id}" data-user_id="${row.id_msg}" ${data == '1' ? 'checked' : ''}>
                        <label class="custom-control-label" for="active-${id}">${data == '1' ? 'Aktif' : 'Nonaktif'}</label>
                    </div>
                    `;
                 }   
