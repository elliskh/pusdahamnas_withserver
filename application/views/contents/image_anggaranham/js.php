...
$(() => {
	...
	$('#table-data').on('click', '.btn-remove', function () {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.post(BASE_URL + 'image_anggaranham/deleteKonten/' + MENU_ID, { id, [CSRF.token_name]: CSRF.hash }).done((res) => {
					if (res.status) {
	...
	$('#table-data').on('change', '.change-active', function () {
		...
		$.post(BASE_URL + 'image_anggaranham/changeKontenActive/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
    		if(res.status==true){
    				toastrSuccess('Sukses', res.message);
    				table.ajax.reload();
    				CSRF.token_name = res.csrf.token_name;
	...
	table = $('#table-data').DataTable({	    
		...
		ajax: {
			url: BASE_URL + 'image_anggaranham/data/' + MENU_ID,
			type: 'get',
			dataType: 'json'
		}, 
		order: [[5, 'desc']],
