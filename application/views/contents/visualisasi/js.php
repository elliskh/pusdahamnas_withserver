...
$(() => {
	...
	$('#table-data').on('click', '.btn-remove', function () {
		...
		let { id, looker_studio } = data;

		Swal.fire({
			title: 'Anda yakin?',
			html: `Anda akan menghapus looker studio "<b>${looker_studio}</b>"!`,
			...
			if (result.isConfirmed) {
				$.post(BASE_URL + 'visualisasi/deleteKonten/' + MENU_ID, { id, [CSRF.token_name]: CSRF.hash }).done((res) => {
					if (res.status) {
	...
	$('#table-data').on('change', '.change-active', function () {
		...
		$.post(BASE_URL + 'visualisasi/changeKontenActive/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
    		if(res.status==true){
    				toastrSuccess('Sukses', res.message);
    				table.ajax.reload();
    				CSRF.token_name = res.csrf.token_name;
	...
	table = $('#table-data').DataTable({	    
		...
		ajax: {
			url: BASE_URL + 'visualisasi/data/' + MENU_ID,
			type: 'get',
			dataType: 'json'
		}, 
		order: [[5, 'desc']],
