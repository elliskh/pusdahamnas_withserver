...
$(() => {
	...
	$('#table-data').on('click', '.btn-remove', function () {
		...
		let { detail_id, detail_deskripsi } = data;

		Swal.fire({
			title: 'Anda yakin?',
			html: `Anda akan menghapus data "<b>${detail_deskripsi}</b>"!`,
			...
			if (result.isConfirmed) {
				$.post(BASE_URL + 'snp/ras_etnis/deleteDetailAnggaran/' + MENU_ID, { detail_id, [CSRF.token_name]: CSRF.hash }).done((res) => {
					if (res.status) {
	...
	$('#table-data').on('change', '.change-ya-tidak', function () {
		...
		$.post(BASE_URL + 'snp/ras_etnis/changeDetailYaTidak/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
	...
	$('#table-data').on('change', '.change-active-public', function () {
		...
		$.post(BASE_URL + 'snp/ras_etnis/changeDetailActivePublic/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
    		if(res.status=='Sukses'){
    				toastrSuccess('Sukses', res.message);
    				table.ajax.reload();
    				CSRF.token_name = res.csrf.token_name;
	...
	$('#table-data').on('click', '.btn-update', function () {
		let tr = $(this).closest('tr');
		let data = table.row(tr).data();
        let url_edit  = '<?= '' . $link . '/' . $menu_id . '' ?>'.replace('tambah','edit_rasetnis');

        window.location = url_edit.replace('<?= $menu_id?>',data.detail_id);
	...
	table = $('#table-data').DataTable({	    
		...
		ajax: {
			url: BASE_URL + 'snp/ras_etnis/detail_data/' + MENU_ID,
			type: 'get',
			dataType: 'json'                        
		}, 
		order: [[5, 'desc']],
		...
		columns: [{
            ...
            ?>
                ...
                <div class="custom-control custom-switch mb-3" dir="ltr">
                    <input type="checkbox" class="custom-control-input change-active-public" name="active-public-${detail_id}" id="active-public-${detail_id}" data-user_id="${detail_id}" ${data == '1' ? 'checked' : ''}>
                    <label class="custom-control-label" for="active-public-${detail_id}">${data == '1' ? 'Aktif' : 'Nonaktif'}</label>
                </div>
                `;                
