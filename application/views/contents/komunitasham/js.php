...
$(() => {
	...
	$('#table-data').on('click', '.btn-remove', function () {
		...
		Swal.fire({
			title: 'Anda yakin?',
			html: `Anda akan menghapus konten "<b>${judul}</b>"!`,
			...
			if (result.isConfirmed) {
				$.post(BASE_URL + 'komunitasham/deleteKonten/' + MENU_ID, { id, [CSRF.token_name]: CSRF.hash }).done((res) => {
					if (res.status) {
	...
	$('#table-data').on('change', '.change-active', function () {
		...
		$.post(BASE_URL + 'komunitasham/changeKontenActive/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
    		if(res.status==true){
    				toastrSuccess('Sukses', res.message);
    				table.ajax.reload();
    				CSRF.token_name = res.csrf.token_name;
	...
	table = $('#table-data').DataTable({	    
		...
		ajax: {
			url: BASE_URL + 'komunitasham/data/' + MENU_ID,
			type: 'get',
			dataType: 'json'
		}, 
		order: [[8, 'desc']],
		...
		columns: [{
           ...
           //decode_id 
              if (type === 'display'){
                return '<a href="'+ BASE_URL + 'home/detail_back_komham/' +  row.id + '" target="_blank" title="Lihat Halaman">'+ data + '</a>';
              }
              return data;
            }
		}, {
