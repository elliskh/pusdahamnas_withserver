		...
		<div class="card">
			<div class="card-body">
				<div class="display_form_registrasi">
					<form id="form-edit-konten" class="form form-horizontal" action="#"  method="POST" autocomplete="off">
						...
						<div class="form-body">
							...
							<div class="row mb-3">
								...
								<div class="col-md-10">
									...
									<input type="text" class="form-control" value="<?=encode_id($data['detail']['idkonten'])?>" name="idkonten" hidden>
									<input type="text" class="form-control" name="judul" placeholder="Judul" value="<?=$data['detail']['judul']?>" required="">
									<input type="hidden" class="form-control" name="deskripsi" placeholder="Deskripsi"  value="<?=$data['detail']['deskripsi']?>">
									<input type="hidden" class="form-control" name="penulis" placeholder="Nama Penulis"  value="<?=$data['detail']['penulis']?>">
									<input type="hidden" class="form-control" name="editor" placeholder="Nama Editor"  value="<?=$data['detail']['editor']?>">
                                    <input type="hidden" class="form-control" name="jenis_konten" placeholder="Jenis konten"  value="0">
							...
							<div class="row mb-3">
								...
								<div class="col-md-10">
									<input type="hidden" class="form-control" name="jumlah_paragraf" placeholder="Jumlah paragraf"  value="<?=$data['detail']['jumlah_paragraf']?>">
									<input type="text" class="form-control" name="sumber_data" placeholder="Sumber data"  value="<?=$data['detail']['sumber_data']?>" required="">
									<span class="validasi"></span>
								</div>
							</div>
							...
							<!--<div class="row mb-3">
								...
								<div class="col-md-10">
									<select name="jenis_konten" class="form-control select2">
										<option value="" selected>Pilih</option>
										<?php foreach ($jenis as $key => $value) { 
										  $xx = $data['detail']['jenis_konten'];
                                          if($value->id==$data['detail']['jenis_konten']){?>
                                            <option value="<?=encode_id($value->id)?>" selected=""><?=$value->nama?></option>
                                        <?php  }else{?>  
											<option value="<?=encode_id($value->id)?>"><?=$value->nama?></option>
										<?php } }?>
									</select>
								</div>
							</div>-->
							<div class="row mb-3">
								<div class="col-md-2">
									<label>Cover Konten</label>
								</div>
								<div class="col-md-10">
									<input type="file" class="form-control" name="cover" id="input_upload2" value="<?=$data['detail']['cover']?>">
									<?php
									if (@$data->cover!="")
									{
										?>
										<div class="note">File Tersimpan : <a target='_blank' href='<?php echo base_url('uploads/cover_konten'.$data['detail']['cover'].'')?>'><?php echo $data['detail']['cover'] ?></a></div>
										<?php
									}
									?>
								</div>
							...
							<div class="row mb-3">
								...
								<!--<div class="col-md-10">
									<textarea name="isikonten" class="form-control" placeholder="Isi Konten" id="" cols="30" rows="10"></textarea>
									<span class="validasi"></span>
								</div>-->
							</div>

                            <div class="form-group">
                                    <textarea id="isi_konten" name="isi_konten" class="tinymce form-control" style="min-height:40vh" required=""><?=$data['detail']['isi_konten']?></textarea>
                            </div>
							<div class="row mb-3">
								<div class="col-md-12 d-flex justify-content-center ">
									<button form="form-edit-konten" type="submit" class="btn btn-lg btn-primary mr-1 mb-1 mt-5 btn-block w-50"> <i class="bx bx-check-circle"></i> Simpan</button>
								</div>
...
<script type="text/javascript">
	$('#form-edit-konten').on('submit', function (e) {
		e.preventDefault();
    ...
    //alert(data);    
		$.ajax({
  	        url: "<?php echo base_url('komunitasham/update');?>",//$(this).prop('action'),
	 		type: "POST",
			data: data, 
			dataType: 'json',
			processData: false,
