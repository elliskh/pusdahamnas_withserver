		...
		<div class="card" style="margin-top: 8%;">
			<div class="card-body">
				<div class="display_form_registrasi">
					<form id="form-add-konten" class="form form-horizontal" action="#"  method="POST" autocomplete="off" enctype="multipart/form-data">
						...
						<div class="form-body">
							<div class="row mt-1 mb-1">
								...
								<div class="col-md-6 col-sm-4">
									<div class="text-right">
										<a href="<?=base_url('home/pegiat_ham')?>" class="btn btn-link text-right">
							...
							<div class="row mb-3">
								...
								<div class="col-md-10">
									...
									<input type="text" class="form-control" name="judul" placeholder="Judul" value="" required="">
                                    <input type="hidden" class="form-control" name="editor" placeholder="Nama Editor"  value="">                                    
									<input type="hidden" class="form-control" name="deskripsi" placeholder="Deskripsi"  value="">
									<input type="hidden" class="form-control" name="penulis" placeholder="Nama Penulis"  value="">
							...
							<div class="row mb-3">
								...
								<div class="col-md-10">
									<input type="text" class="form-control" name="sumberdata" id="sumberdata" placeholder="Sumber data"  value="" required="">
                                    <input type="hidden" class="form-control" name="jml_paragraf" placeholder="Jumlah paragraf"  value="">
                                    <input type="hidden" class="form-control" name="jenis_konten" placeholder="Isi konten"  value="0">
									<span class="validasi"></span>
								</div>
							</div>
							...
							<div class="row mb-3">
								...
								<div class="col-md-10">
									...
									{
										?>
										<div class="note">File Tersimpan : <a target='_blank' href='<?php echo base_url('uploads/komunitasham/'.@$data->cover.'')?>'><?php echo @$data->cover ?></a></div>
										<?php
									}
									?>
								</div>
								...
								<div class="col-md-10">
									<select name="jenis_konten" class="form-control">
										...
										<?php foreach ($jenis as $key => $value) { ?>
											<option value="<?=encode_id($value->id)?>" <?=@$data->id_jenis_dokumen==$value->id?'selected':''?>><?=$value->nama?></option>
										<?php } ?>
									</select>
								</div>
							</div>
							<div class="row mb-3">
								...
								<div class="col-md-10">
									<input type="file" class="form-control" name="cover" id="input_upload2" required="">
									<?php
									if (@$data->cover!="")
									{
										?>
							...
							<!--<div class="row mb-3">
								<div class="col-md-2">
									<label>Jumah Paragraf</label>
								</div>
								<div class="col-md-10">
									<input type="text" class="form-control" name="jml_paragraf" placeholder="Jumlah paragraf"  value="">
									<span class="validasi"></span>
								</div>
							</div>-->
							<div class="row mb-3">
                            ...
                            <div class="form-group">
                                    <textarea id="isi_konten" name="isi_konten" class="tinymce form-control" style="min-height:40vh" required=""></textarea>
                            </div>
							<div class="row mb-3">
...
<script type="text/javascript">
    ...
    //alert(data);    
		$.ajax({
  	        url: "<?php echo base_url('komunitasham/simpan');?>",//$(this).prop('action'),
	 		type: "POST",
			data: data, 
			dataType: 'json',
			processData: false,
			...
			success: (res) => {
				...
				if (res.status=='sukses') {
				    ...
				    alert("Terima kasih, Konten anda akan di lakukan review terlebih dahulu");
                    //location.reload()
					//toastrSuccess('success_messages', "Pendaftaran berhasil");
                    window.location = "<?=base_url('home/pegiat_ham')?>";
			...
			//	table.ajax.reload();
             ...
             error: (res) => {//alert("Terjadi kesalahan di server3!");
              if(JSON.stringify(res.status)==500){
                ...
                alert("Berhasil simpan data ");history.go(-1);
              }
			/*error: (res) => {
                alert("Terjadi kesalahan di server!");
