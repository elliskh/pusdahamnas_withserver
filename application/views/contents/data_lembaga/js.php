...
$(document).ready(function(){
	...
	$('#table-data').on('change', '.change-active', function () {
		...
		$.post(BASE_URL + 'data_lembaga/changeKontenActive/' + MENU_ID, { id, val: (checked ? 1 : 0), [CSRF.token_name]: CSRF.hash}).done((res) => {
    		if(res.status==true){
...
function load_table(){
    ...
    var table = $('#table-data').DataTable({
        ...
        order: [],
        ajax: {
            url: "<?= base_url('data_lembaga/get_data/'.$menu_id)?>",
            type: "POST",
            data:{
                [csrfName]: csrfHash,
...
function hapus(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?php echo base_url('data_lembaga/hapus/'.$menu_id) ?>",
                    type: "GET",
                    dataType: 'JSON',
                    data: {
                        id: id
