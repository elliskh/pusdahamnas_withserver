    ...
    <div class="row">
      ...
      <div class="col-12" style="margin-top:8%;">
        <div class="card">
          ...
          <?php 
              $looker = '';
              if ($this->uri->segment(2)=='visualisasi'){
                $looker = decrypt($this->uri->segment(3));
                $where = "is_active = 1 AND looker_studio = $looker";
                $dtx = $this->db->where('looker_studio',$looker)->get('tb_visualisasi')->result(); 
                   foreach($dtx as $dd){
                      $dt = $dd->looker_studio;
                   }      
                ?>  
                <?php if($looker!=$dt){?>        
                    <iframe id="isu33" name="isu33" class="" style="margin-right: -0.2%;float: right;margin-top: -1%;display:block" width="100%;" height="650px;" src="" frameborder="0" style="border:0" allowfullscreen></iframe>
                <?php }else{?>
                    <iframe id="isu33" name="isu33" class="" style="margin-right: -0.2%;float: right;margin-top: -1%;display:block" width="100%;" height="650px;" src="https://lookerstudio.google.com/embed/reporting/<?= $looker ?>" frameborder="0" style="border:0" allowfullscreen></iframe>
               <?php     }?>
          <?php  }else{ 
                  echo "<script>alert('Tidak ada data untuk ditampilkan!')</script>"; 
              }?>
          </div>
        </div>
      </div>
