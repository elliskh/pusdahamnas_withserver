...
class Tb_data_dok extends CI_Model
{
    ...
    {
        $this->db->from("v_tb_basisdata a");
        if (@$_POST['id_topik_hak']) {
            $this->db->where('id_topik_hak', decode_id($_POST['id_topik_hak']));
        }
        if (@$_POST['id_unit_kerja']) {
            if ($_POST['id_unit_kerja']!="all")
            {
                $this->db->where('unit_kerja', $_POST['id_unit_kerja']);
            }
        }
        if (@$_POST['id_lembaga']) {
            if ($_POST['id_lembaga']!="all")
            {
                 $this->db->where('id_jenis_dokumen', decode_id($_POST['id_lembaga']));
            }
        }
        $i = 0;
