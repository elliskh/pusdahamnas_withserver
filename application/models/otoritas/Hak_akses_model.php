...
class Hak_akses_model extends CI_Model
{
	...
	public function getMainMenuOfRole($role_id)
	{
		$this->db->select('
        menus.*,
        GROUP_CONCAT(CASE WHEN menu_role.is_active = "1" THEN menu_role.action_id END) as actions,
        menu_role.is_active as menu_role_active,
        menu_role.role_id
        ');
		$this->db->from('menus');
		...
		$this->db->group_by('menus.id, menus.urutan, menus.parent_id, menus.deleted_at, menus.is_active, menu_role.is_active, menu_role.role_id');
	...
	public function updateHakAkses($role_id, $menu_id, $action_id, $val)
	{
		...
		if ($check->num_rows() == 0) {
			...
			$this->db->update('menu_role', ['is_active' => $val]);
		}
