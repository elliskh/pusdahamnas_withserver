...
class Image_komunitasham extends MY_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->_auth();
		$this->_path = 'image_komunitasham/';
		$this->_table = 'tb_image_komunitas_ham';
        $this->link = base_url('image_komunitasham');
		//=========================================================//

		$this->load->model($this->_path . 'Image_komunitasham_model', 'user');
	...
	//	])->get('roles')->result();
		...
		///	'roles' => $roles,
			//=========================================================//
			'breadcrumb' => ['Dashboard', 'Gambar Pegiat', 'index'],
			'modals' => [],
            /*'modals' => [
	...
	///	checkAccessAjax($this->_path, $menu_id, 3);
		...
		$menu_id   = post('menu_id');
            ...
            if($this->upload->do_upload('gambar')) { 
                        	...
                        	$insert = $this->db->insert("tb_image_komunitas_ham", $data);
        					if($insert){
        					   echo "<script>alert('Tambah data berhasil')</script>";
                            	redirect(base_url('image_komunitasham/index/'.$menu_id));
        						$this->session->set_flashdata('success', 'Berhasil menambahkan.');
                            }else{
	...
	//	checkPermission($this->_path, $menu_id, 2);
		...
		$config 			= [
			...
			'menu_id' 		=> $menu_id,
			'link' 			=> $this->link,
			'menu_active' 	=> 'Gambar Komunitas HAM',
			'menu_open' 	=> null,
			'path' 			=> $this->_path,
			...
			//=========================================================//
			'breadcrumb' 	=> [
				'Dashboard', 'Gambar Komunitas HAM', 'index'
			],
			'modals' 		=> [],
			'plugins' => [
				'select2' => true
	...
	public function edit_konten($menu_id = null)
	{
        ...
        $data['detail'] = $this->db->select('a.id, a.gambar, a.deskripsi, a.is_active')->where('a.id', $id)->get('tb_image_komunitas_ham a')->row_array();
		...
		$config 			= [
			...
			//=========================================================//
			'breadcrumb' 	=> [
				'Dashboard', 'Edit Gambar Komunitas HAM', 'index'
			],
			'modals' 		=> [],
			'plugins' => [
				'select2' => true
		...
		$deskripsi = post('deskripsi');
           ...
           /* if ( ! $this->upload->do_upload()) {
            ...
            if($this->upload->do_upload('gambar')) { 
                        	...
                        	$update = $this->db->where('id',$id)->update("tb_image_komunitas_ham", $data_update);
        						//$this->session->set_flashdata('success', 'Berhasil menambahkan.');
        						///redirect(base_url('image_slide/index/'.$menu_id));
                            		response([
                            			'status' => 'sukses',
	...
	public function changeKontenActive($menu_id = null)
	{
      ...
      ///  $check_actived = $this->user->checkKontenActive();
    		...
    		$this->db->where('id', $id);
    	$update = $this->db->update('tb_image_komunitas_ham', [
    			'is_active' => $val,
    		]);
        if($update){
    		response([
	...
	public function deleteKonten($menu_id = null)
	{
		...
		$this->db->where('id', $id);
		$this->db->update('tb_image_komunitas_ham', ['is_active' => '0','deleted_at' => 'Y']);
