...
class Audit_ham extends MY_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->_auth();
		$this->_path = 'audit_ham/';
		$this->_table = 'tb_audit_ham';
        $this->link = base_url('audit_ham');
		//=========================================================//
		$this->load->model($this->_path . 'Auditham_model', 'user');
		$this->load->model($this->_path . 'Detail_auditham_model', 'detail');
	...
	//	])->get('roles')->result();
		...
		$config = [
			'title' => 'Data Audit HAM',
			'menu_id' => $menu_id,
			'link' 			=> $this->link.'/tambah',
			'menu_active' => 'Data Audit HAM',
			'menu_open' => null,
			'path' => $this->_path,
		...
		///	'roles' => $roles,
			//=========================================================//
			'breadcrumb' => ['Dashboard', 'Data Audit HAM', 'index'],
			'modals' => [],
            /*'modals' => [
	...
	public function detail_auditham($menu_id = null) {
		method('get');
		///checkPermission($this->_path, $menu_id, 1);
		//=========================================================//
        $id = decrypt($this->uri->segment(3));
        $data['detail'] = $this->db->select('*')->where('id', $id)->get('tb_audit_ham')->row_array();
	...
	//	])->get('roles')->result();
		...
		$config = [
			'title' => 'Detail Audit HAM',
			'menu_id' => $menu_id,
			'link' 			=> $this->link.'/detail_tambah',
			'menu_active' => 'Detail Audit HAM',
			'menu_open' => null,
			'path' => $this->_path,
            'data' => $data,
			'contents' =>  $this->_path . 'detail_auditham',
			'script_js' 		=> $this->_path . 'detail_auditham_js',
			//=========================================================//
		///	'roles' => $roles,
			//=========================================================//
			'breadcrumb' => ['Dashboard', 'Detail Audit HAM', 'Detail'],
			'modals' => [],
            /*'modals' => [
				$this->_path . 'modals/tambah',
				$this->_path . 'modals/ubah',
	...
	public function simpan()
	{
		...
		$nama_audit   = $this->security->xss_clean(post('nama_audit'));
		$deskripsi    = $this->security->xss_clean(post('deskripsi'));
        
		$insert = $this->db->insert('tb_audit_ham', [
 	...
 	public function detail_simpan()
	{
		...
		$insert = $this->db->insert('tb_audit_ham_detail', [
			'id_audit'       => decrypt($id),
			'bab'            => $bab,
			'deskripsi'      => $deskripsi,
			'ya_tidak'       => '0',
	...
	//	checkPermission($this->_path, $menu_id, 2);
		...
		$config 			= [
			...
			//=========================================================//
			'breadcrumb' 	=> [
				'Dashboard', 'Detail Audit HAM', 'Detail'
			],
			'modals' 		=> [],
			'plugins' => [
				'select2' => true
	...
	public function edit_auditham($menu_id = null)
	{
	...
	///	checkAccessAjax($this->_path, $menu_id, 3);
		...
		$nama_audit = post('nama_audit');
		$deskripsi  = post('deskripsi');

		$this->db->where('id', 1);
		$this->db->update('tb_audit_ham', [
	...
	public function edit_detail($menu_id = null)
	{
        ...
        $data['detail'] = $this->db->select('*')->where('id', $id)->get('tb_audit_ham_detail')->row_array();
		...
		$config 			= [
			...
			'jenis' 		=> $jenis,
            'data'          => $data,
			'menu_active' 	=> 'Data Detail Audit HAM',
			'menu_open' 	=> null,
			'path' 			=> $this->_path,
			...
			//=========================================================//
			'breadcrumb' 	=> [
				'Dashboard', 'Detail Audit HAM', 'index'
			],
			'modals' 		=> [],
			'plugins' => [
				'select2' => true
	...
	public function detail_edit_auditham($menu_id = null)
	{
		method('get');
		//checkPermission($this->_path, $menu_id, 2);
        $id = decrypt($this->uri->segment(3));
		...
		$config 			= [
			'title' 		=> 'Edit Data Detail Audit HAM',
			'menu_id' 		=> $menu_id,
			'link' 			=> $this->link,
			'jenis' 		=> $jenis,
            'data'          => $data,
	...
	///	checkAccessAjax($this->_path, $menu_id, 3);
		...
		$this->db->where('id', $id);
		$this->db->update('tb_audit_ham_detail', [
		///	'id_user'        => decrypt($this->session->userdata('id')),
            'bab'            => $bab,
			'deskripsi'      => $deskripsi,
		]);
	...
	public function changeAuditHamActive($menu_id = null)
	{
        $xhr = TRUE; // if we decide to use XHR ajax
      ...
      ///  $check_actived = $this->user->checkKontenActive();
    		...
    		$this->db->where('id', $id);
    	$update = $this->db->update('tb_audit_ham', [
    			'is_active' => $val,
    		]);
        if($update){
    		response([
	...
	//	checkAccessAjax($this->_path, $menu_id, 3);
		...
		$val = post('val');
            ...
            $update = $this->db->update('tb_audit_ham_detail', [
    			'ya_tidak' => $val,
    		]);
        if($update){
    		response([
	...
	public function deleteAuditHam($menu_id = null)
	{
	//	checkAccessAjax($this->_path, $menu_id, 4);
		...
		$this->db->where('id', $id);
		$this->db->update('tb_audit_ham', ['is_active' => '0','deleted_at' => 'Y']);
	...
	public function deleteDetailAudit($menu_id = null)
	{
	//	checkAccessAjax($this->_path, $menu_id, 4);
		...
		$this->db->where('id', $id);
		$this->db->update('tb_audit_ham_detail', ['is_active' => '0','deleted_at' => 'Y']);
