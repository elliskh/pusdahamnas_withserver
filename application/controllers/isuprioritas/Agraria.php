...
class Agraria extends MY_Controller
{
	public function __construct()
	{
		...
		$this->_table = '';
		$this->link = base_url('isuprioritas/agraria/');
		//=========================================================//

		$this->load->model('menu/Menu_model', 'menu');
		$this->load->model('isuprioritas/agraria/Tb_data_agraria', 'Tb_data_agraria');
	}
	...
	public function index($menu_id = null)
	{
		...
		$config 			= [
			'title' 		=> 'Data Konflik Agraria',
			'menu_id' 		=> $menu_id,
			'link' 			=> $this->link.'/tambah',
			'menu_active' 	=> 'Data Konflik Agraria',
			'menu_open' 	=> null,
			'path' 			=> $this->_path,
	...
	//	checkPermission($this->_path, $menu_id, 1);
        ...
        foreach ($list as $row) {
            ...
            $isi[] = '<b>'.$row->judul.'</b>';
            $isi[] = $row->deskripsi;
			$aksi .= '<a class="btn btn-sm btn-primary btn-block" href="'.base_url('isuprioritas/agraria/edit/'.encode_id($row->id)).'/'.$menu_id.'"> <i class="fa fa-edit"></i> Edit</a>';
	...
	public function simpan(){
		...
		$cek = $this->db->from('tb_isuagraria')->where('id',$id)->get();
		if ($cek->num_rows()> 0) {
			$this->db->where('id',$id)->update('tb_isuagraria',$insert);
		}else{
			$this->db->insert('tb_isuagraria',$insert);
		...
		if ($_FILES['poster']['size'] != 0 && $_FILES['poster']['error'] == 0)
			...
			redirect(base_url('isuprioritas/agraria/index/'.$menu_id));
	}
