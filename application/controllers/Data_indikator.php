...
class Data_indikator extends MY_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->_auth();
		$this->_path = 'data_indikator/';
		$this->_table = '';
		$this->link = base_url('data_indikator');
		//=========================================================//

		$this->load->model('menu/Menu_model', 'menu');
		$this->load->model('data_indikator/tb_data_indikator', 'tb_data_indikator');
	}
	...
	public function index($menu_id = null)
	{
		...
		$config 			= [
			...
			'menu_active' 	=> 'Data Indikator HAM',
			'menu_open' 	=> null,
			'path' 			=> $this->_path,
			...
			//=========================================================//
			'breadcrumb' 	=> [
				'Utama', 'Data Indikator HAM', 'index'
			],
			'modals' 		=> [],
			'plugins' 		=> [
				'select2' => true,
	...
	function get_data($menu_id) {
		// method('post');
		checkPermission($this->_path, $menu_id, 1);
        $list = $this->tb_data_indikator->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $row) {
            $no++;
			...
			$aksi .= '<a class="btn btn-sm btn-primary btn-block" href="'.base_url('data_indikator/edit/'.encode_id($row->id_lembaga)).'/'.$menu_id.'"> <i class="fa fa-edit"></i> Edit</a>'; 
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->tb_data_indikator->count_all(),
            "recordsFiltered" => $this->tb_data_indikator->count_filtered(),
            "data" => $data,
        );
	...
	public function tambah($menu_id = null)
	{
		...
		$hak = $this->db->get('ref_indikator_dokumen')->result();
		$jenis = $this->db->get('ref_jenis_indikator_dokumen')->result();
		$media = $this->db->get('ref_media_indikator_dokumen')->result();
		$status = $this->db->get('ref_status_indikator_dokumen')->result();
		$subyek = $this->db->get('ref_subyek_indikator_dokumen')->result();
		$lembaga = $this->db->get('ref_indikator')->result();
		$config 			= [
			'hak' 		=> $hak,
			'lembaga' 		=> $lembaga,
			'jenis' 		=> $jenis,
			...
			'status' 		=> $status,
			'subyek' 		=> $subyek,
			'title' 		=> 'Tambah Data Indikator HAM',
			'menu_id' 		=> $menu_id,
			'link' 			=> $this->link,
			'menu_active' 	=> 'Data Indikator HAM',
			'menu_open' 	=> null,
	...
	public function edit($id,$menu_id = null)
	{
		...
		checkPermission($this->_path, $menu_id, 2);
		$id = decode_id($id);
		$data = $this->db->from('tb_indikator')->where('id_lembaga',$id)->get()->row();        
		$hak = $this->db->get('ref_indikator_dokumen')->result();
		$jenis = $this->db->get('ref_jenis_indikator_dokumen')->result();
		$media = $this->db->get('ref_media_indikator_dokumen')->result();
		$status = $this->db->get('ref_status_indikator_dokumen')->result();
	...
	public function simpan(){
		...
		$cek = $this->db->from('tb_indikator')->where('id_lembaga',$id)->get();
		if ($cek->num_rows()> 0) {
			$insert['updated_at']=date('Y-m-d H:i:s');
			$this->db->where('id_lembaga',$id)->update('tb_indikator',$insert);
		}else{
			$insert['created_at']=date('Y-m-d H:i:s');
			$this->db->insert('tb_indikator',$insert);
		...
		// 	}
			...
			redirect(base_url('data_indikator/index/'.$menu_id));
	}

	public function hapus($menu_id){
		method('get');
		$id=decode_id($this->input->get('id'));
		$this->db->where('id_lembaga',$id)->update("tb_indikator", ['deleted_at'=> date("Y-m-d H:i:s")]);
		if($this->db->affected_rows()>0){
