...
class Isuagraria extends MY_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->_auth();
		$this->_path = 'isuagraria/';
		$this->_table = '';
		$this->link = base_url('isuagraria');
		//=========================================================//

		$this->load->model('menu/Menu_model', 'menu');
		$this->load->model('isuagraria/tb_data_isuagraria', 'tb_data_isuagraria');
	}
	...
	function get_data($menu_id) {
		// method('post');
		checkPermission($this->_path, $menu_id, 1);
        $list = $this->tb_data_isuagraria->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $row) {
            $no++;
            ...
            $isi[] = '<b>'.$row->judul.'</b>';
            $isi[] = $row->deskripsi;
			$aksi .= '<a class="btn btn-sm btn-primary btn-block" href="'.base_url('isuagraria/edit/'.encode_id($row->id)).'/'.$menu_id.'"> <i class="fa fa-edit"></i> Edit</a>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->tb_data_isuagraria->count_all(),
            "recordsFiltered" => $this->tb_data_isuagraria->count_filtered(),
            "data" => $data,
        );
