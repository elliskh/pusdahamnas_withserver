...
class Data_anggaran extends MY_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->_auth();
		$this->_path = 'data_anggaran/';
		$this->_table = '';
		$this->link = base_url('data_anggaran');
		//=========================================================//

		$this->load->model('menu/Menu_model', 'menu');
		$this->load->model('data_anggaran/tb_data_anggaran', 'tb_data_anggaran');
	}
	...
	function get_data($menu_id) {
		// method('post');
		checkPermission($this->_path, $menu_id, 1);
        $list = $this->tb_data_anggaran->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $row) {
            $no++;
            ...
            $isi[] = '<b>'.$row->judul.'</b>';
            $isi[] = $row->deskripsi;
			$aksi .= '<a class="btn btn-sm btn-primary btn-block" href="'.base_url('data_anggaran/edit/'.encode_id($row->id)).'/'.$menu_id.'"> <i class="fa fa-edit"></i> Edit</a>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->tb_data_anggaran->count_all(),
            "recordsFiltered" => $this->tb_data_anggaran->count_filtered(),
            "data" => $data,
        );
	...
	public function tambah($menu_id = null)
	{
		...
		$hak = $this->db->get('ref_anggaran_dokumen')->result();
		$jenis = $this->db->get('ref_anggaran_jenis_dokumen')->result();
		$media = $this->db->get('ref_anggaran_media_dokumen')->result();
		$status = $this->db->get('ref_anggaran_status_dokumen')->result();
		$subyek = $this->db->get('ref_anggaran_subyek_dokumen')->result();
        
        if ($this->session->userdata('id_lembaga')>0)
        {
            $this->db->where('id',$this->session->userdata('id_lembaga'));
        }
		$lembaga = $this->db->get('ref_anggaran')->result();
		$config 			= [
			'hak' 		=> $hak,
			'lembaga' 		=> $lembaga,
			'jenis' 		=> $jenis,
	...
	public function edit($id,$menu_id = null)
	{
		...
		checkPermission($this->_path, $menu_id, 2);
		$id = decode_id($id);
		$data = $this->db->from('tb_anggaran')->where('id',$id)->get()->row();
		$hak = $this->db->get('ref_anggaran_dokumen')->result();
		$jenis = $this->db->get('ref_anggaran_jenis_dokumen')->result();
		$media = $this->db->get('ref_anggaran_media_dokumen')->result();
		$status = $this->db->get('ref_anggaran_status_dokumen')->result();
	...
	public function simpan(){
		...
		if ($_FILES['poster']['size'] != 0 && $_FILES['poster']['error'] == 0)
			...
			redirect(base_url('data_anggaran/index/'.$menu_id));
	}

	public function hapus($menu_id){
		method('get');
		$id=decode_id($this->input->get('id'));
		$this->db->where('id',$id)->update("tb_anggaran", ['deleted_at'=> date("Y-m-d H:i:s")]);
		if($this->db->affected_rows()>0){
