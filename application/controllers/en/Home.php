...
class Home extends MX_Controller
{
  ...
  //       $this->template->display_front($view, $data);
		redirect(base_url('en/home/data'));
	}
	...
	public function data($id=null) {
		...
		$data = [
			...
			'agenda' => $this->db->order_by('start','desc')->get('tb_event',3)->result(),
		...
		$view = [
            'title' => "Home",
            'content' => 'front/data_en',
            'js' => 'front/data_js',
            'css' => 'front/data_css'
        ];

        $this->template->display_front_en($view, $data);
	}
	...
	public function agenda() {
        $data = [
			'agenda' => $this->db->order_by('start','desc')->from('tb_event')->get()->result()
		];
	...
	public function detail_agenda($id) {
        $data = [
			...
			'agenda' =>$this->db->from('tb_event')->order_by('id_event','desc')->limit(5)->get()->result()
		];
